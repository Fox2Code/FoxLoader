...
public final class CertificateHelper {
    ...
    private static final String[] certificates = new String[]{
            // Let's Encrypt
            "ISRG-Root-X1.pem", "ISRG-Root-X2.pem",
            // DigiCert
            "DigiCert-Global-Root-CA.pem",
            "DigiCert-Global-Root-G2.pem", "DigiCert-Global-Root-G3.pem",
            // Google Trust services
            "GTS-Root-R1.pem", "GTS-Root-R2.pem",
            "GTS-Root-R3.pem", "GTS-Root-R4.pem",
            // USERTrust
    ...
    public static void initializeSafe() {
        try {
            ...
            throw new RuntimeException("Failed to initialize CertificateHelper", e);
        }
        if (factory == null) {
            throw new RuntimeException("Failed to initialize CertificateHelper");
        }
    }
    ...
    public static void initialize() throws Exception {
        ...
        X509TrustManager flEncryptTrustManager = makeTrustManager();
        ...
        sslContext.init(null, new TrustManager[]{
                new X509DualTrustManager(flEncryptTrustManager, systemTrustManager)
        }, null);
    ...
    public static void install(HttpsURLConnection httpsURLConnection) throws Exception {
        initialize();
        if (httpsURLConnection.getSSLSocketFactory() != factory) {
    ...
    private static X509TrustManager makeTrustManager()
        ...
        for (String certificate : CertificateHelper.certificates) {
            certificates.addAll(certificateFactory.generateCertificates(Objects.requireNonNull(
                    CertificateHelper.class.getResourceAsStream(prefix + certificate), certificate)));
        }
